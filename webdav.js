...
async function getLyric(musicItem) {
  ...
  const lrcPath =
    lastDot === -1
      ? musicItem.id + ".lrc"
      : musicItem.id.substring(0, lastDot) + ".lrc";
...
async function getRecommendSheetsByTag(tag, page) {
  ...
  try {
    const items = await client.getDirectoryContents(tag.id);
    const folders = items.filter((item) => item.type === "directory");

    console.log(
      `getRecommendSheetsByTag 在 ${tag.id} 下找到 ${folders.length} 个文件夹`,
    );

    const sheets = folders.map((folder) => ({
      id: folder.filename,
      platform: "WebDAV",
      title: folder.basename,
      artwork: "",
    ...
    console.error(`获取歌单列表失败 ${tag.id}:`, error);
    return { isEnd: true, data: [] };
  }
}
...
async function getMusicSheetInfo(sheetItem, page) {
  ...
  if (typeof sheetItem === "string") {
    folderPath = sheetItem;
  } else if (sheetItem && sheetItem.id) {
    folderPath = sheetItem.id;
  } else {
    console.error("getMusicSheetInfo: 无效的参数", sheetItem);
    return { isEnd: true, musicList: [] };
  }
  ...
  console.log("getMusicSheetInfo 正在读取路径:", folderPath);
  ...
  try {
    ...
    console.log(`getMusicSheetInfo 找到 ${fileItems.length} 个音频文件`);

    const sheetTitle =
      typeof sheetItem === "object" && sheetItem.title ? sheetItem.title : "";
    ...
    console.error(`获取歌单详情失败 ${folderPath}:`, error);
    return { isEnd: true, musicList: [] };
  }
}
...
module.exports = {
  ...
  version: "1.1.1",
  author: "异飨客",
  description: "连接 WebDAV 服务器播放音乐，支持导入歌单",
  userVariables: [
    {
      key: "url",
      name: "WebDAV地址",
